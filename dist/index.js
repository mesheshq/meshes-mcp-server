...
//  WORKSPACES
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
server.registerTool("meshes_list_workspaces", {
    title: "List Workspaces",
    description: "List all workspaces in the Meshes organization. Workspaces are tenant-scoped containers holding connections, rules, and event logs. In multi-tenant apps, each customer typically has one workspace.",
    inputSchema: {},
    annotations: {
        readOnlyHint: true,
...
server.registerTool("meshes_get_workspace", {
    title: "Get Workspace",
    description: "Get details of a specific workspace by UUID, including its publishable key for client-side event ingestion.",
    inputSchema: {
...
server.registerTool("meshes_create_workspace", {
    title: "Create Workspace",
    description: "Create a new workspace. Typically used when onboarding a new tenant in a multi-tenant SaaS app.",
    inputSchema: {
        name: z
            .string()
            .min(3)
            .max(50)
            .describe("Workspace name (3-50 chars, alphanumeric/space/underscore/dash)"),
        description: z
            .string()
            .max(1024)
            .optional()
...
server.registerTool("meshes_update_workspace", {
    ...
    try {
        return toolOk(await client.updateWorkspace(workspace_id, { name, description }));
    }
    catch (e) {
        return toolError(e);
    }
});
server.registerTool("meshes_get_workspace_connections", {
...
server.registerTool("meshes_get_workspace_rules", {
...
server.registerTool("meshes_update_connection", {
    ...
    try {
        return toolOk(await client.updateConnection(connection_id, { name, metadata, hidden }));
    }
    catch (e) {
        return toolError(e);
    }
